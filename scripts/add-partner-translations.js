...
/**
 * One-time fix: add FR and ES locale entries to already-seeded partners.
 *
 * Run after restarting Strapi (schema change removes description_fr/description_es).
 *
 * Usage:
 *   STRAPI_TOKEN=<api-token> node scripts/add-partner-translations.js
 */
...
// Keyed by Webflow ID → { fr, es } descriptions
const TRANSLATIONS = {
...
async function main() {
  ...
  if (!map.partners) {
    console.error('No partners in map. Run seed-partners.js first.');
    process.exit(1);
  }

  for (const [wid, entry] of Object.entries(map.partners)) {
    const t = TRANSLATIONS[wid];
    if (!t) { console.log(`⚠ No translations found for wid ${wid} (${entry.name}) — skipping`); continue; }

    process.stdout.write(`→ ${entry.name} … `);

    await api('PUT', `/api/partners/${entry.documentId}?locale=fr`, {
      data: { description: t.fr },
    });

    await api('PUT', `/api/partners/${entry.documentId}?locale=es`, {
      data: { description: t.es },
    });

    console.log('done');
  }

  console.log('\nAll partner translations added.');
}
